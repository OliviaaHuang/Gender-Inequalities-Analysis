...
def set_bar3d(x_data, data, year):
    ...
    bar = Bar3D(init_opts=opts.InitOpts(
        width='1500px', height='1000px', page_title='Explore the courses that boys and girls in %s' % year)
    )
    bar.add(
        series_name="",
        ...
        zaxis3d_opts=opts.Axis3DOpts(type_="value", name='number of student'),
    )
    bar.set_global_opts(
        ...
        title_opts=opts.TitleOpts(title="Explore the number of boys and girls like the course")
    )
    bar.render('Number of men and women who liked the course in %s.html' % year)
